...
- Has access to two variables already in scope: \`page\` (a Playwright Page, not yet navigated) and \`expect\` (Playwright's expect, e.g. expect(locator).toBeVisible()).
...
- Sets a reasonable timeout expectation implicitly by using Playwright's built-in auto-waiting; do not add manual sleep/waitForTimeout unless truly necessary.
